...
function ViewScoreComponent({ memberId, guestId }) {
  ...
  useEffect(() => {
    axios.get(`http://ec2-3-36-96-61.ap-northeast-2.compute.amazonaws.com:8080/api/v1/portfolios/profits/${memberId}`)
        .then((response) => {
    ...
    if (guestId) {
      axios.get(`http://ec2-3-36-96-61.ap-northeast-2.compute.amazonaws.com:8080/api/v1/portfolios/profits/${guestId}`)
          .then((response) => {
            const data = response.data;
            if (data) {
